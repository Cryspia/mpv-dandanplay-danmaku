...
- **分季识别**：续作会匹配到对应季，而不是落回第一季（`S02E01`、`第二季`、`2nd Season`、罗马音剧名均可）。
- **剧场版 / 电视动画、年份区分**：电影和剧集同名时，按文件本身的形态和文件名里的 `(2016)` 之类年份判断。
- **开关即重试**：搜索失败时，按 `F10` 关掉再打开即可重新搜索；请求本身也会先自动重试几次。
- **手动模糊搜索**（Ctrl+F10）：两级选择器 —— 输入关键词 → 选番剧（显示类型与首播年份）→ 选剧集。
...
搜索失败时（网络抖动、代理故障），按 `F10` 关掉再打开即可重新搜索。
...
2. **第一级 —— 番剧选择器**：列出所有匹配到的番剧（含类型、首播年份、集数），用 `↑↓` / `PgUp/PgDn` / `1-9` 选定。
...
2. 没有候选分数够高 → fallback 查 `aliases.json`，命中「Magic Workshop → 尖帽子的魔法工房」；
3. 把别名追加到查询序列里重新搜索、重新打分 → 命中 → 加载弹幕。
